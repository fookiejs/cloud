...
export default Vue.extend({
  ...
  methods: {
    ...
    create: async function() {
      ...
      for (let key of Object.keys(body)) {
        ...
        } else if (this.model.schema[key].input === 'phone') {
          body[key] = body[key].replace(/[\(\)\- ]+/g, '');
        } else if (this.model.schema[key].input === 'file') {
          let file = this.body[key];
          if (!file) return;
          let q;
          if (this.$store.state.system_user && this.$store.state.system_user['attributes']['name'] === 'admin')
            q = '&companyId=' + this.$store.state.selectedCompany;
    ...
    getAddress: async function(Text, field) {
      ...
      try {
        ...
        this.$refs.mymap[0].mapObject.panTo(Results[0].Place.Geometry.Point, { animate: true });
        this.$refs.mymap[0].mapObject.fitBounds(_.chunk(Summary.ResultBBox.reverse(), 2));
      } catch (e) {
      }
    },
  },
  watch: {
    ...
    dialog(val) {
      ...
      setTimeout(() => {
        if (this.$refs.mymap) this.$refs.mymap.mapObject.invalidateSize();
      }, 300);
    },
  },
});
    ...
    v-card-text(v-for='(field, i) in fields' :key="i+'postfiledi_'")
      v-form(v-model='valid' :ref="'form_'+model.name")
        ...
        div.mapClass(v-if="field.input==='map'")
          ...
          l-map(ref="mymap" :center.sync='body[i] ' :zoom='20'
